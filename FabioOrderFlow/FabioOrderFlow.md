...
Il recorder scrive righe JSON con prefisso `FofHistoricalContext` e schema `fof-historical-cumulative-context-v4`: candle/footprint storici del capture range, value area, VWAP, POC di candle, livelli footprint e risposta storica `CumulativeTrade` con ticks interni. Mantiene una sola richiesta ATAS pendente alla volta. Il report `../docs/research/historical-cumulative-context-inventory-2026-08-04.md` documenta che ATAS puo' restituire anche eventi fuori dalla finestra richiesta; i parser devono quindi filtrare esplicitamente `inside-request` quando serve una popolazione temporale chiusa. Non riceve raw trade storici arbitrari e quindi non sostituisce il recorder live `fof-session-observation-v2`; serve a costruire case study storici con contesto d'asta da candle e aggregati ATAS.
