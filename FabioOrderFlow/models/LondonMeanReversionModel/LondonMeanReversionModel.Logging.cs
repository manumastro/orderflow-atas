    ...
    internal sealed partial class LondonMeanReversionModule
    {
        private void ResetStudyLog()
        {
            ...
            try
            {
                ...
                File.WriteAllText(_historicalLogPath, $"[Source=Historical] [Seq={++_historicalLogSequence}] [WriteItaly={writeItaly}] [WriteUtc={writeUtc:yyyy-MM-dd HH:mm:ss.fff}] [HISTORICAL_STUDY_START] Mode={mode}, AggregatedStudyEnabled={_historicalStudyActive}, DailyDebugEnabled={_dailyHistoricalDebugLogsEnabled}, HistoricalIntrabarEnabled={EnableHistoricalIntrabarFromCumulativeTrades}, OperationalLogs=MR_*, StudyLogs=DAY_STUDY_*, StudyLogsDoNotTrade=True, HistoricalReplayUsesOperationalEntryPath=True, HistoricalIntrabarReplay=LIVE_INTRABAR_REPLAY_EMULATION, MinAggressionVolume={MinAggressionVolume:F0}, MinRewardRiskToTarget2={MinRewardRiskToTarget2:F2}, DynamicStopMaxValueAreaRiskPct={DynamicStopMaxValueAreaRiskPct:F2}, OperationalEntryTimeoutSeconds={OperationalEntryTimeoutSeconds}, CreatedItaly={MarketTimeZones.ToItaly(DateTime.UtcNow):yyyy-MM-dd HH:mm:ss}{Environment.NewLine}");
